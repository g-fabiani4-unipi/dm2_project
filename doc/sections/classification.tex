...
For Support Vector Machines (SVMs), we scaled numerical features using RobustScaler, as our data contains outliers. We deemed it  We tested the linear kernel and the Radial Basis Function kernel separately to exploit sklearn's optimized LinearSVC implementation.
...
We also tried implementing early stopping: the macro F1 sank to 0.236, but the accuracy increased slightly, reaching 0.425. 
...
Implementing early stopping leads to a drop in macro F1 to 0.728, as it performs slightly worse on minority classes. 

\subsection{Conclusions}
The performances of the different models for the two tasks are summarized in Table \ref{tab:rating_models} and \ref{tab:titletype_models}.
...
All models achieved great results for the more popular classes, with an F1 around 0.9, while the results for the minority classes varied from around 0.4 to a robust 0.7 for the best models. In particular, the best models achieved great results for \textit{videogame}, and were better at separating \textit{tvMovie} and \textit{tvMiniSeries} from their more popular counterparts, \textit{movie} and \textit{tvSeries}. Ensemble methods have clearly outperformed the rest.

With regard to \textit{rating}, the performance of every model reflects the class distribution, which resembles a left-skewed Gaussian: performances peak for class (7,8], the mode, and quickly degrade. The models do not have a chance to generalize on the lowest and the highest classes, especially those in the 'tail', due to lack of representation. The best models, however, managed to concentrate their confusion on neighboring classes, especially around the peak of the distribution.

Finally, there have been some challenges tied to overfitting; future work could explore undersampling and oversampling techniques to improve generalization.