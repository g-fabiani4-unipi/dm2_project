...
For the global structural approach, we extracted mean, standard deviation and Catch22 features from the raw signals (without amplitude scaling) and then computed the Euclidean distance on this new feature matrix (which was normalized using a Standard Scaler).
...
Since in our understanding phase we were able to recognize two distinct patterns, we first of all ran \textbf{K-Means} with \textit{k}=2 and obtained results very similar to those previously discussed. This clustering obtained a silhouette score of 0.85 and was able to identify 159 `late bloomers'.

We then experimented with different linkage criteria for \textbf{hierarchical clustering}, selecting in the end \textbf{average linkage}, which yielded the best dendrogram (Fig. \ref{fig:tsclust_dendrogram}).
...
We then run \textbf{Principal Component Analysis} on the dataset and visualized clusters on the two first components, which together explain more than 60\% of variance (Fig. \ref{fig:hier_pca}, cluster medoids are showed in black).

Inspecting the PCA Eigenvectors, we found out that the first component is mainly concerned with timestamps from around the first week, which explains why records from C2 (and some records from C1) have positive values in this component, while the `late bloomers' from C3 have very negative values.
Moreover, observing the distribution of records from C1 along the first component, it is clear why more than half of them have a negative silhouette score, since they clearly should belong to C2.
...
We also visualized the clusters using \textbf{tSNE} on the \textbf{DTW distance matrix}.
We choose this projection for its ability of preserving the local structure of the dataset.
While \textbf{tSNE} was able to convey some information on the numeric relative dimensions of different clusters, and was able to visualize clusters nicely, in the end it proved less interesting, because of its inability of revealing the global structure of the dataset and of conveying the variable density of our clusters.

Inspection of the PCA visualization convinced us that extraction of clusters with a density-based method was viable.
Since a series of runs with different radii failed to return any interesting clusters, we decided to pursue a different approach: we run \textbf{OPTICS} (with \texttt{minPts=4}) to completion on a reduced version of the dataset, using the first 5 components created by PCA (which capture more than 80\% of variance). We then visualized the reachability plot and choose an appropriate radius with which to run \textbf{DBSCAN} using the reachability distances computed by OPTICS.
...
In this analysis we concentrated our attention on \textbf{instant hits} from C3 and \textbf{'late bloomers'} from C2 returned by the hierarchical clustering.
...
We ran \textbf{hierarchical clustering} on time series structural features, experimenting with different linkage criteria and selecting the one that gave use the best dendrogram (\textbf{Ward}).
...
From Fig. \ref{fig:tsclust_structure.png} we can see that this clustering method was also able to detect the patterns observed previously, albeit in a less marked way. C1 and C2 (which are later merged in a single cluster) are mainly made up of `late bloomers', while C3 and C4 group together instant hits (C3 and C4 are sisters).
