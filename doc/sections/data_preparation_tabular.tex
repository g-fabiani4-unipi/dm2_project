...
Since the distribution of \textit{runtimeMinutes} depends on \textit{titleType} (cf. Fig \ref{fig:boxplot_runtime_tts}), and the same holds true for \textit{writerCredits}, we imputed their missing values using the median value within each respective class.
...
\begin{figure}
    ...
    \caption{Distribution of runtimes across \textit{titleTypes}.}
    \label{fig:boxplot_runtime_tts}
\end{figure}


\subsection{Clustering}
\begin{figure}
    \includegraphics[width=\columnwidth]{../results/images//understanding_clustering.png}
    \caption{Hierarchical clustering results on tabular data by runtime and number of votes.}
    \label{fig:tabular_clustering}
\end{figure}
