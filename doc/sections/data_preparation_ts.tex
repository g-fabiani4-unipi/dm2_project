...
We then splitted our initial dataset, reserving 25\% of records for testing, and we conducted the rest of our analysis exclusively on the training set, as to prevent data leakage.

\subsubsection{Univariate analysis of variables}
The distribution of number of votes, awards, nominations and total gross income show all a pronounced right skew, which is not unexpected. 90\% of records had received awards or nominations.
...
As for the distribution w.r.t. genres, any given movie can have more than one genre and the most popular genre was \emph{Drama} (48\%), followed by \emph{Comedy} (36\%), \emph{Action} and \emph{Adventure} (each of these 32\%).
...
Fig. \ref{fig:ts_feat_correlation} reports the Spearman correlation coefficient for pairs of numeric features.
Besides obviously being correlated with each other, both \textit{awards} and \textit{nominations} are positively correlated with both the \textit{numVotes} and \textit{rating}.
We also observe a moderate to strong positive correlation between the estimated gross income and number of votes.
Perhaps surprisingly, \textit{totalGrossIncome} is only weakly correlated with \textit{rating}.

Positive correlation of the \textit{numVotes} with \textit{rating} is something we didn't observe in the IMDB dataset, where, on the contrary, these two features were weakly negatively correlated.
...
Firstly, our time series generally show slightly negative trend and some seasonality with a cycle over around 14 timestamps: this observation was confirmed when we extracted Catch22 features from the time series and were able to verify that most of them had a periodicity of 13 or 14.
This finding, which we initially took at face value, was somewhat puzzling.  We expected a day of week seasonality, with peaks in the weekends, so it didn't make sense for peaks to occur every other week.
Surely enough, when we checked against the original data on Box Office Mojo, we were able to confirm that the observed peaks are, in fact, the effect of day of week seasonality, and that the time series were altered before we received them, either during the extraction or during the preprocessing.
